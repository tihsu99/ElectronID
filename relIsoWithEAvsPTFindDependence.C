...
const TString hinName    = "relIsoWithEA";
const TString xAxisTitle = "pT[GeV]";
const TString yAxisTitle = "relIsoWithEA";
...
void hoeEnergyFindDependence(TString region){
  ...
  system("mkdir -p relIsoWithEA_calibration/" + region + "/");
  etaLabel = region;
  TString finName = "relIsoWithEAvspt_" + region + ".root";
  ...
  if( !useCutoff )
    fracString = "markers: mean of relIsoWithEA in pt slices";
  TLatex *lat2 = new TLatex(0.15, 0.75, fracString);
  lat2->SetNDC(kTRUE);
  lat2->SetTextSize(0.03);
  lat2->Draw();

  TString slopeString = TString::Format("f(Pt)=A+B/Pt, B=%.2e #pm %.2e",B, dB);
  ...
  c1->Print("relIsoWithEA_calibration/" + region + "/relIsoWithEAvsPt.png");
  c1->Print("relIsoWithEA_calibration/" + region + "/relIsoWithEAvsPt.pdf");
  // Other plots
...
void plot2D(TH2F *hist, TString region){
  ...
  cmsText.DrawLatex(0.1, 0.93, "CMS");
  ...
  extraText.DrawLatex(0.18, 0.93, "Preliminary");

  TLatex *lat = new TLatex(0.7, 0.8, etaLabel);
  lat->SetNDC(kTRUE);
  lat->Draw();
  c2->Print("relIsoWithEA_calibration/" + region + "/relIsoWithEAvsPt_2D.png");
  c2->Print("relIsoWithEA_calibration/" + region + "/relIsoWithEAvsPt_2D.pdf");
...
void plotVariable(TH2F *hist, TString region){
  ...
  hist1D->GetYaxis()->SetTitleOffset(2);
  hist1D->Draw("hist");
  ...
  c3->Print("relIsoWithEA_calibration/" + region + "/relIsoWithEAvsPt_variable.png");
  c3->Print("relIsoWithEA_calibration/" + region + "/relIsoWithEAvsPt_variable.pdf");
