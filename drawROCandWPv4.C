...
const TString tmvaFileNameBarrel[nROCs] = {
  "./trainingData/training_results_barrel_pass1_" + dateTag + "/TMVA_training_results_barrel_pass1_" + dateTag + ".root",
  "./trainingData/training_results_barrel_pass2_" + dateTag + "/TMVA_training_results_barrel_pass2_" + dateTag + ".root",
  "./trainingData/training_results_barrel_pass3_" + dateTag + "/TMVA_training_results_barrel_pass3_" + dateTag + ".root",
  "./trainingData/training_results_barrel_pass4_" + dateTag + "/TMVA_training_results_barrel_pass4_" + dateTag + ".root"
};

const TString tmvaFileNameEndcap[nROCs] ={
  "./trainingData/training_results_endcap_pass1_" + dateTag + "/TMVA_training_results_endcap_pass1_" + dateTag + ".root",
  "./trainingData/training_results_endcap_pass2_" + dateTag + "/TMVA_training_results_endcap_pass2_" + dateTag + ".root",
  "./trainingData/training_results_endcap_pass3_" + dateTag + "/TMVA_training_results_endcap_pass3_" + dateTag + ".root",
  "./trainingData/training_results_endcap_pass4_" + dateTag + "/TMVA_training_results_endcap_pass4_" + dateTag + ".root"
};
...
const int nWP = 1;
const TString cutFileNamesBarrelSet1[nWP] = {"cut_repository/cuts_barrel_" + dateTag + "_WP_Veto.root"};
const TString cutFileNamesEndcapSet1[nWP] = {"cut_repository/cuts_endcap_" + dateTag + "_WP_Veto.root"};

// Set 2
const int markerColorSet2 = kOrange;
const int markerStyleSet2 = 20;
const TString legendSet2 = "WP_Loose";
const TString cutFileNamesBarrelSet2[nWP] = {"cut_repository/cuts_barrel_" + dateTag + "_WP_Loose.root"};
const TString cutFileNamesEndcapSet2[nWP] = {"cut_repository/cuts_endcap_" + dateTag + "_WP_Loose.root"};

// Set 3
const int markerColorSet3 = kBlue;
const int markerStyleSet3 = 20;
const TString legendSet3 = "WP_Medium";
const TString cutFileNamesBarrelSet3[nWP] = {"cut_repository/cuts_barrel_" + dateTag + "_WP_Medium.root"};
const TString cutFileNamesEndcapSet3[nWP] = {"cut_repository/cuts_endcap_" + dateTag + "_WP_Medium.root"};
...
const TString cutFileNamesBarrelSet4[nWP] = {"cut_repository/cuts_barrel_" + dateTag + "_WP_Tight.root"};
const TString cutFileNamesEndcapSet4[nWP] = {"cut_repository/cuts_endcap_" + dateTag + "_WP_Tight.root"};
...
void overlayWorkingPoints(TCanvas *c1, TTree *signalTree, TTree *backgroundTree, 
			  const TString *cutFileNames, int markerColor, int markerStyle, 
			  TLegend *leg, const TString legendText, bool drawBarrel);
void findEfficiencies(TTree *signalTree, TTree *backgroundTree,
			float &effSignal, float &effBackground, 
			VarCut *cutObject, int iWorkingPoint, bool drawBarrel);
//
// Main function
//
void drawROCandWPv4(bool drawBarrel){
  ...
  if(!tmvaFile1 || !tmvaFile2 || !tmvaFile3 || !tmvaFile4) assert(0);
  bazinga("Step #3");
  // 
  //  Draw the ROC curve
  //
  ...
  for(int iBin = 1; iBin <=100; ++iBin){
    ...
    if (drawBarrel){
      if(1<=iBin && iBin<=75)         hROC->SetBinContent(iBin, hROC3->GetBinContent(iBin)) ;
      else if(76 <= iBin && iBin<=89)	hROC->SetBinContent(iBin, hROC2->GetBinContent(iBin)) ;
      else if(90 <=iBin && iBin<=100)	hROC->SetBinContent(iBin, hROC1->GetBinContent(iBin)) ;
    } else {
      if(1<=iBin && iBin<=70)        	hROC->SetBinContent(iBin, hROC3->GetBinContent(iBin)) ;
      else if(71 <= iBin && iBin<=89) hROC->SetBinContent(iBin, hROC2->GetBinContent(iBin)) ;
      else if(90 <=iBin && iBin<=100)	hROC->SetBinContent(iBin, hROC1->GetBinContent(iBin)) ;
    }
  }
  ...
  const TString signalTreeName = Opt::signalTreeName;
  const TString fnameBackground = drawBarrel ? Opt::fnameBackgroundBarrel : Opt::fnameBackgroundEndcap;
  const TString backgroundTreeName = Opt::backgroundTreeName;
  ...
  if( 1 ){
    ...
    leg->SetFillStyle(0);
    leg->SetBorderSize(0);
    overlayWorkingPoints(c1, signalTree, backgroundTree, cutFileNamesSet1, markerColorSet1, markerStyleSet1, leg, legendSet1, drawBarrel);
    bazinga("Step #9");
    overlayWorkingPoints(c1, signalTree, backgroundTree, cutFileNamesSet2, markerColorSet2, markerStyleSet2, leg, legendSet2, drawBarrel);
    bazinga("Step #10");
    overlayWorkingPoints(c1, signalTree, backgroundTree, cutFileNamesSet3, markerColorSet3, markerStyleSet3, leg, legendSet3, drawBarrel);
    bazinga("Step #11");
    overlayWorkingPoints(c1, signalTree, backgroundTree, cutFileNamesSet4, markerColorSet4, markerStyleSet4, leg, legendSet4, drawBarrel);
    bazinga("Step #12");
    leg->Draw("same");
    ...
    TString outname = (TString)"figures/plot_ROCandWP_" + (drawBarrel? "barrel" : "endcap") + ".png";
    bazinga("Step #13");
    c1->Print(outname);
...
void   overlayWorkingPoints(TCanvas *c1, TTree *signalTree, TTree *backgroundTree, 
  ...
  for(int iwp = 0; iwp<nWP; iwp++){
    ...
    if(!cutObject) assert(0);
    
    // Compute the efficiencies
    float effSignal, effBackground;
    findEfficiencies(signalTree, backgroundTree, effSignal, effBackground, cutObject, iwp, drawBarrel);
...
void findEfficiencies(TTree *signalTree, TTree *backgroundTree,
		      ...
		      int iWorkingPoint, bool drawBarrel){

  TCut etaCut = drawBarrel ? Opt::etaCutBarrel : Opt::etaCutEndcap;
