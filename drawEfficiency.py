...
workingPoints = {
  'default':     [workingPoint('Veto',          'cuts_barrel_' + dateTag + '_WP_Veto',   'cuts_endcap_' + dateTag + '_WP_Veto',   2, 9,  2, 3),
                  workingPoint('Loose',         'cuts_barrel_' + dateTag + '_WP_Loose',  'cuts_endcap_' + dateTag + '_WP_Loose',  4, 46, 1, 1),
                  workingPoint('Medium',        'cuts_barrel_' + dateTag + '_WP_Medium', 'cuts_endcap_' + dateTag + '_WP_Medium', 6, 30, 1, 1),
                  workingPoint('Tight',         'cuts_barrel_' + dateTag + '_WP_Tight',  'cuts_endcap_' + dateTag + '_WP_Tight',  8, 42, 1, 1)],
...
def drawFromTree(tree, selectionCuts, varName, histName, binning):
  hist = ROOT.TH1F(histName, histName, len(binning)-1, numpy.array(binning))
  hist.Sumw2()
  cutString  = "genWeight*kinWeight*(%s)" % selectionCuts.GetTitle().replace('scaled','')  ### Note: temporary, should find a way to handle both HoverEscaled as HoverE
...
# Main function
#
def drawEfficiency(mode, tag):
  for region in ['endcap','barrel'] if mode != 'eta' else ['full']:
    ...
    for selectVar in ["", "expectedMissingInnerHits", "full5x5_sigmaIetaIeta", "dEtaSeed", "dPhiIn", "hOverE", "relIsoWithEA", "ooEmooP"]:
      ...
      for wp in workingPoints[tag]:
        ...
        sigNum = drawFromTree(signalTree,     (signalCuts + selectionCuts),     varName, 'sigNum'+ wp.name.split()[0], binning)
        sigDen = drawFromTree(signalTree,     (signalCuts),                     varName, 'sigDen'+ wp.name.split()[0], binning)
        bgNum  = drawFromTree(backgroundTree, (backgroundCuts + selectionCuts), varName, 'bgNum' + wp.name.split()[0], binning)
        bgDen  = drawFromTree(backgroundTree, (backgroundCuts),                 varName, 'bgDen' + wp.name.split()[0], binning)
...
for tag in ['compare2016']:
  for mode in  ['pt', 'pt_2TeV', 'eta', 'nvtx']:
    drawEfficiency(mode, tag)
